...
| Коэффициент корреляции Пирсона | L2 Ранжирование | ![Pearson correlation coefficient (PCC)](/PCC_Formula.png) -  позволяет измерить сходство между пользователями путем вычисления корреляции между соответствующими рейтингами двух пользователей. PCC находится в диапазоне от -1 до 1 (-1 отрицательная корреляция, 1 - идентичная). В формуле: a и b — разные пользователи, а rai и rbi — это рейтинг этого пользователя для элемента i(твита) |
...
| Amazon Personalize | Рекомендательные технологии | Amazon Personalize позволяет быстро создавать и развертывать эффективные решения, а также выполнять сегментацию клиентов в любом масштабе, используя технологию ML. |
| Apache Mahout | Рекомендательные технологии | Apache Mahout свободный проект, обеспечивающий распределенную, масштабируемую среду для алгортмов машинного обучения с фокусом на линейную алгебру. |
| Surprise(Python) | Рекомендательные технологии | Surprise (Simple Python RecommendatIon System Engine) - это Python-библиотека, разработанная для создания и оценки рекомендательных систем. |
### Схема проекта
...
| --- | --- |
| Auth Service |  |
| User Service | реплицки master-slave. snapshot. Auth сервис при регистрации делает запрос на добавление пользователя в User, гарантирует, что пользователь будет создан при регистрации. |
| Tweet Service |  реплицки master-slave. snapshot. при создании новых твитов они записываются в outbox, чтобы после передать в Search Service, гарантирует, что твит будет внесён в поиск и рекомендации. |
| Search Service | (в Tweet Service связано) |
| Comment Service | реплицки master-slave. snapshot |
| Community Service | реплицки master-slave. snapshot |
| Stats Service | clickhouse-backup. репликация(MergeTree) |
| File Service | S3 встроенное резервирование |
