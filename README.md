...
Trie или префиксное дерево — это особый вид дерева поиска, в котором для ключей узлов используются строки (SSTable Attached Indexes (SAI) в  cassandra db)

Каждый узел дерева содержит один символ и указатели на дочерние узлы. Ключ узла вычисляется как путь от корня до узла.
...
Область применения: тестирование отказоустойчивости, поиск уязвимостей, проверка надежности и устойчивости системы.

Мотивационная часть: подход предполагает проведение контролируемых экспериментов, имитирующих реальные сценарии сбоев, такие как внезапные скачки трафика, перебои в работе сети или отказы оборудования. Такие контролируемые эксперименты, могут выявить потенциально слабые места системы и повысить устойчивость. Примеры, что можно сделать: CPU spikes, сетевые задержки, потерю пакетов, имитация утечек памяти, потери пакетов, отключения или ухудшения производительности микросервисов(можно целиком отключить ДЦ), задержки при доступе к базе данных и т.д. 

Инструменты: Gremlin, Chaos Monkey, Pumba.


Gzip

Мотивационная часть: Самый распространённый алгоритм сжатия. Сжимает без потерь, с хорошей степенью сжатия (настраивается от 1 до 9, по умолчанию 6) и быстрой распаковкой. Он основан на алгоритме DEFLATE, который использует комбинацию алгоритма LZ77 и алгоритма Хаффмана. Алгоритм LZ77 заменяет повторные вхождения данных на «ссылки». Простой и эффективный, подойдёт в большинстве случаев. gzip: 50-95% сжатия в зависимости от контента. В среднем по вебу 65-80%.

Инструменты: ngx_http_gzip_module, "compress/gzip" package в go.


Terraform, Ansible.

Область применения: Инструмент с открытым исходным кодом для управления инфраструктурой как кодом (IaC). Позволяет описывать и управлять инфраструктурой с помощью декларативного языка конфигурации, подойдет для конфигурации CDN, серверов. Например, можно быстро обновить утилиту на множестве машин, а не вручную на каждой.
